...
- **Meta Ads**: skill `meta-ads-coreos` (SDK oficial facebook-business)
- **Google Ads**: skill `google-ads-coreos` (SDK oficial google-ads)
- **Google Analytics**: skill `ga4-coreos` (GA4 Data API)
...
O setup:
1. Detecta se o CoreOS está instalado (lê `_memoria/empresa.md` se existir)
2. Verifica quais skills de execução estão disponíveis (meta-ads-coreos, google-ads-coreos, ga4-coreos)
...
| `/ads-coreos relatorio` | Dashboard HTML com benchmarks BR | Entrega pro cliente (semanal/mensal) |
| `/ads-coreos auditoria` | Análise profunda com Quality Gates | Revisão mensal |
| `/ads-coreos historico` | Registra e consulta otimizações e hipóteses | Após cada ação |
...
**Arquivo:** `aprendizados.md` (na raiz da skill, `.claude/skills/ads-coreos/aprendizados.md`)
...
Cada skill de execução (meta-ads-coreos, google-ads-coreos, ga4-coreos) tem seu próprio `aprendizados.md` pra regras específicas da plataforma. O do ads-coreos é pra regras gerais (formato de relatório, preferências de visualização, etc).

## Regras gerais

1. **NUNCA usar MCPs**: toda execução DEVE ser via scripts Python das skills de execução (meta-ads-coreos, google-ads-coreos, ga4-coreos). Nunca usar fb-ads-mcp-server, adloop ou qualquer outro MCP de terceiro. Isso garante consistência e independência.
...
```bash
$PYTHON .claude/skills/meta-ads-coreos/scripts/read.py accounts
```
...
```bash
# Meta Ads
ls .claude/skills/meta-ads-coreos/SKILL.md 2>/dev/null && echo "META_OK"

# Google Ads
ls .claude/skills/google-ads-coreos/SKILL.md 2>/dev/null && echo "GOOGLE_OK"

# GA4
ls .claude/skills/ga4-coreos/SKILL.md 2>/dev/null && echo "GA4_OK"
```

Se algum resultado vier vazio, a skill de execução correspondente não está
presente nesse repo — isso não deveria acontecer num negócio criado com o
CoreOS (elas vêm por padrão). Orientar o usuário a rodar `/atualizar` ou
verificar se a pasta `.claude/skills/` foi copiada por completo.
