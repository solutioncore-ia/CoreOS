...
Skill completa para gestao de Google Ads via SDK oficial (`google-ads`). Executa queries GAQL para leitura, e mutate operations para escrita. Equivalente ao meta-ads-coreos para o ecossistema Google.

**IMPORTANTE: Esta skill e o braco de execucao. Para inteligencia (diagnostico, auditoria, estrategia), use a skill ads-coreos como camada acima.**
...
Checar se existe `.claude/skills/google-ads-coreos/.env`. Se NAO existir, criar com o template:

```
# Google Ads CoreOS — Configuracao
...
**Fallback**: Se existir `.claude/skills/google-ads-coreos/google-ads.yaml`, o SDK carrega dele automaticamente.
...
```bash
python3 .claude/skills/google-ads-coreos/scripts/setup.py oauth
```
...
```bash
python3 .claude/skills/google-ads-coreos/scripts/setup.py full
```
...
**Arquivo:** `.claude/skills/google-ads-coreos/contas.yaml`
...
Todos os scripts estao em `.claude/skills/google-ads-coreos/scripts/`. O padrao e:
...
**Arquivo:** `aprendizados.md` (na raiz da skill, `.claude/skills/google-ads-coreos/aprendizados.md`)
