#!/usr/bin/env python3
"""
GA4 CoreOS - Biblioteca compartilhada
Auth (google-analytics-data SDK), .env loader, output helpers, error handling
Suporta Service Account e OAuth2 (compartilhado com google-ads-coreos)
"""
...
_ENV_SEARCH_PATHS = [
    os.path.expanduser("~/.claude/skills/ga4-coreos/.env"),
    os.path.join(os.path.dirname(os.path.dirname(os.path.dirname(os.path.abspath(__file__)))), ".env"),
]

# Fallback: buscar credenciais OAuth do google-ads-coreos
_GOOGLE_ADS_ENV_PATHS = [
    os.path.expanduser("~/.claude/skills/google-ads-coreos/.env"),
]
...
def _load_google_ads_env():
    """Tenta carregar credenciais OAuth do google-ads-coreos como fallback."""
...
def init_client():
    ...
    2. OAuth2 do proprio ga4-coreos (GA4_CLIENT_ID, GA4_CLIENT_SECRET, GA4_REFRESH_TOKEN)
    3. OAuth2 do google-ads-coreos (GOOGLE_ADS_CLIENT_ID, GOOGLE_ADS_CLIENT_SECRET, GOOGLE_ADS_REFRESH_TOKEN)
    ...
    # --- Modo 3: OAuth2 do google-ads-coreos (compartilhado) ---
    ...
    if gads_client_id and gads_client_secret and gads_refresh_token:
        ...
        _client = BetaAnalyticsDataClient(credentials=credentials)
        print(f"Client inicializado via OAuth2 google-ads-coreos ({gads_env})", file=sys.stderr)
        return _client
    ...
    print("  Opcao 3 (Google Ads): Configure google-ads-coreos e as credenciais serao compartilhadas", file=sys.stderr)
    print("", file=sys.stderr)
    print("  Crie o arquivo ~/.claude/skills/ga4-coreos/.env", file=sys.stderr)
    print("  Ou rode: /ga4-coreos setup", file=sys.stderr)
    sys.exit(1)
