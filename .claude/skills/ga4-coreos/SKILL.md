...
**IMPORTANTE: Esta skill e o braco de execucao para GA4. Para inteligencia (diagnostico, auditoria, estrategia), use a skill ads-coreos como camada acima.**

**IMPORTANTE: Esta skill e separada do google-ads-coreos porque muita gente usa Meta Ads + GA4 sem Google Ads. Porem, se o usuario ja tem google-ads-coreos configurado, as credenciais OAuth podem ser compartilhadas.**
...
Checar se existe `.claude/skills/ga4-coreos/.env`. Se NAO existir, criar com o template:

```
# GA4 CoreOS — Configuracao
...
# MODO 2: OAuth2 (compartilhado com google-ads-coreos)
# Se voce ja tem google-ads-coreos configurado, pode reusar as credenciais:
# GA4_CLIENT_ID=""
# GA4_CLIENT_SECRET=""
# GA4_REFRESH_TOKEN=""
# Ou deixe em branco e o script busca automaticamente no .env do google-ads-coreos.
```
...
**Modo 2 (OAuth2):** Se o usuario ja tem `google-ads-coreos` configurado com OAuth, os scripts buscam automaticamente as credenciais de `.claude/skills/google-ads-coreos/.env`. Nao precisa configurar nada extra.
...
**Arquivo:** `.claude/skills/ga4-coreos/contas.yaml`
...
Todos os scripts estao em `.claude/skills/ga4-coreos/scripts/`. O padrao e:
...
1. **Somente leitura** — GA4 CoreOS e uma skill de consulta, NAO modifica dados
