...
**IMPORTANTE:** Ler `references/setup-meta-app.md` ANTES de comecar o setup. Esse arquivo contem o passo a passo completo para criar o app no Meta Developer Dashboard, gerar o token e resolver problemas. Se o usuário mandar prints ou tiver duvidas sobre alguma tela do Facebook, consultar esse arquivo para orientar.

### 1. Verificar dependencias

```bash
python3 .claude/skills/meta-ads-coreos/scripts/setup.py
```

### 2. Verificar .env

Checar se existe `.claude/skills/meta-ads-coreos/.env`. Se NAO existir, criar com o template:

```
# Meta Ads CoreOS — Configuracao
...
**Arquivo:** `.claude/skills/meta-ads-coreos/contas.yaml`
...
Todos os scripts estao em `.claude/skills/meta-ads-coreos/scripts/`. O padrao e:
...
**Arquivo:** `aprendizados.md` (na raiz da skill, `.claude/skills/meta-ads-coreos/aprendizados.md`)
