#!/usr/bin/env python3
"""
Meta Ads CoreOS - Biblioteca compartilhada
...
_ENV_SEARCH_PATHS = [
    os.path.expanduser("~/.claude/skills/meta-ads-coreos/.env"),
...
def init_api():
    ...
    if not token:
        ...
        print("  Crie o arquivo ~/.claude/skills/meta-ads-coreos/.env com:", file=sys.stderr)
        print('  META_ADS_TOKEN="seu-token-aqui"', file=sys.stderr)
        print("", file=sys.stderr)
        print("  Ou rode: /meta-ads-coreos setup", file=sys.stderr)
        sys.exit(1)
